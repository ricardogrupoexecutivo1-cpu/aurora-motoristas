...
export default function ClienteTokenPage() {
  ...
  async function carregar() {
    ...
    if (result.ok) {
      const encontrado = (result.services || []).find(
        (item: any) => String(item.client_token) === token
      );
  ...
  async function confirmar() {
    ...
    const response = await fetch("/api/services/confirm", {
      method: "POST",
      headers: {
  ...
  if (loading) {
    return <main style={{ padding: 30 }}>Carregando serviço do cliente...</main>;
  }
      ...
      <section style={{ maxWidth: 820, margin: "0 auto", background: "white", padding: 24, borderRadius: 24 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
          <div>
            ...
            <h1 style={{ fontSize: 34, marginTop: 8 }}>Serviço do cliente</h1>
          </div>
        ...
        {confirmado && (
          <div style={{ marginTop: 16, background: "#dcfce7", padding: 14, borderRadius: 16, fontWeight: 900 }}>
        ...
        <div style={{ marginTop: 24, display: "grid", gap: 14 }}>
          ...
          <Info titulo="Valor do serviço" valor={moeda(Number(service.client_amount || 0))} destaque />
          <Info titulo="Status" valor={service.status || "pendente"} />
        </div>

        {service.status !== "cliente_confirmou" && (
          <button
            onClick={confirmar}
            style={{
              marginTop: 24,
