...
export default function ClientePainelPage() {
  ...
  async function carregar() {
    ...
    if (result.ok) {
      ...
      setServices(lista.filter((item: any) => item.client_id === clientId));
    }
  ...
  const resumo = useMemo(() => {
    return {
      ...
      pendentes: services.filter((item) => item.status !== "cliente_confirmou").length,
      totalContratado: services.reduce((acc, item) => acc + Number(item.client_amount || 0), 0),
    };
  }, [services]);
  ...
  if (loading) {
    return (
      <AuroraProtected allowedRoles={["cliente"]}>
        <main style={{ padding: 30 }}>Carregando painel do cliente...</main>
      </AuroraProtected>
    );
  }
        ...
        <section style={{ maxWidth: 1180, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
            <div>
              ...
              <h1 style={{ fontSize: 36, marginTop: 8 }}>Painel do Cliente</h1>
              <p style={{ color: "#475569", marginTop: 8 }}>
                Aqui o cliente visualiza somente os serviços vinculados ao seu client_id.
              </p>
            </div>
          ...
          <div style={{ display: "grid", gap: 14, gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", marginTop: 24 }}>
            ...
            <Card titulo="Total contratado" valor={moeda(resumo.totalContratado)} destaque />
          </div>

          <div style={{ marginTop: 28, background: "white", borderRadius: 24, padding: 20, overflowX: "auto" }}>
            <h2 style={{ fontSize: 24, marginBottom: 14 }}>Meus serviços contratados</h2>
              ...
              <tbody>
                {services.map((item) => (
                  <tr key={item.id} style={{ borderTop: "1px solid #e2e8f0" }}>
                    ...
                    <td style={td}>
                      <Badge texto={item.status || "pendente"} />
                    </td>
                    ...
                    <td style={td}>
                      <strong>{moeda(Number(item.client_amount || 0))}</strong>
                    </td>
                  </tr>
                ))}
                ...
                {services.length === 0 && (
                  <tr>
                    <td style={td} colSpan={6}>
                      Nenhum serviço vinculado ao seu cliente ainda.
                    </td>
                  </tr>
                )}
              </tbody>
...
function Badge({ texto }: { texto: string }) {
  const ok = texto === "cliente_confirmou";
