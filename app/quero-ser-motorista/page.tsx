...
function hasNota(service: ServiceRow) {
  return Boolean(
    getNotaNumero(service) || getNotaEmissao(service) || getNotaVencimento(service)
  );
}
...
export default function AdminServicosPage() {
  ...
  async function reabrirServico(
    ...
    await atualizarService(
      ...
      `${getDisplayOS(service)} foi reaberto para ${destino}.`
    );
  }

  async function marcarNotaRecebida(service: ServiceRow) {
    await atualizarService(
      service,
      {
        status_nota_fiscal: "recebida",
      },
      `Marcando nota de ${getDisplayOS(service)} como recebida...`,
      `Nota fiscal de ${getDisplayOS(service)} marcada como recebida.`
    );
  }

  async function marcarNotaPendente(service: ServiceRow) {
    await atualizarService(
      service,
      {
        status_nota_fiscal: "pendente",
      },
      `Marcando nota de ${getDisplayOS(service)} como pendente...`,
      `Nota fiscal de ${getDisplayOS(service)} marcada como pendente.`
    );
  }

  async function limparStatusFiscal(service: ServiceRow) {
    await atualizarService(
      service,
      {
        status_nota_fiscal: null,
      },
      `Limpando status fiscal de ${getDisplayOS(service)}...`,
      `Status fiscal de ${getDisplayOS(service)} limpo com sucesso.`
    );
  }
  ...
  const totais = useMemo(() => {
    ...
    const fiscal = services.reduce(
      (acc, service) => {
        const notaStatus = getNotaStatusFiscal(service);

        if (!hasNota(service)) acc.semNota += 1;
        if (notaStatus.label === "A vencer") acc.aVencer += 1;
        if (notaStatus.label === "Vencida") acc.vencidas += 1;
        if (notaStatus.label === "Recebida") acc.recebidas += 1;
    ...
    <main style={pageStyle}>
      <div style={containerStyle}>
        <section style={heroStyle}>
          ...
          >
            ...
            <p style={heroText}>
              ...
              aguardando pagamento, pagos, histÃ³rico protegido e situaÃ§Ã£o fiscal.
            </p>
        ...
        <section style={statsGridStyle}>
          <StatCard
            label="Sem nota"
            value={String(totais.semNota)}
            help="ServiÃ§os sem dados fiscais informados."
          />
          <StatCard
            label="Notas a vencer"
            value={String(totais.aVencer)}
            help="Com vencimento futuro e sem baixa."
          />
          <StatCard
            label="Notas vencidas"
            value={String(totais.vencidas)}
            help="Precisam de aÃ§Ã£o rÃ¡pida."
          />
          <StatCard
            label="Notas recebidas"
            value={String(totais.recebidas)}
            help="Baixadas/recebidas no fiscal."
          />
        </section>
        ...
        >
          <div style={panelStyle}>
            ...
            <p style={sectionText}>
              Leitura visual imediata para entender onde a base estÃ¡
              concentrada.
            </p>
          ...
          <div style={panelStyle}>
            ...
            <div style={adminNoteStyle}>
              Este painel foi pensado para leitura rÃ¡pida, conferÃªncia geral,
              correÃ§Ãµes administrativas e controle fiscal sem misturar a visÃ£o
              operacional enxuta com a visÃ£o master.
            </div>
          </div>
        </section>
                ...
                <article key={service.id} style={serviceCardStyle}>
                  ...
                  <div style={fiscalBoxStyle}>
                    ...
                    <div style={actionsBoxStyle}>
                      <div style={actionsTitleStyle}>AÃ§Ã£o rÃ¡pida fiscal</div>

                      <div style={actionsRowStyle}>
                        <button
                          type="button"
                          disabled={disabled}
                          onClick={() => marcarNotaRecebida(service)}
                          style={{
                            ...actionPrimaryButtonStyle,
                            opacity: disabled ? 0.6 : 1,
                          }}
                        >
                          Marcar nota como recebida
                        </button>

                        <button
                          type="button"
                          disabled={disabled}
                          onClick={() => marcarNotaPendente(service)}
                          style={{
                            ...actionButtonStyle,
                            opacity: disabled ? 0.6 : 1,
                          }}
                        >
                          Marcar nota como pendente
                        </button>

                        <button
                          type="button"
                          disabled={disabled}
                          onClick={() => limparStatusFiscal(service)}
                          style={{
                            ...actionButtonStyle,
                            opacity: disabled ? 0.6 : 1,
                          }}
                        >
                          Limpar status fiscal
                        </button>
                      </div>

                      <div style={helperTextStyle}>
                        {disabled
                          ? "Atualizando fiscal deste serviÃ§o..."
                          : "O status fiscal ajuda a separar nota recebida, pendente, vencida e serviÃ§os sem nota."}
                      </div>
                    </div>
                  </div>
